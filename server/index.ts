...
app.post('/api/saveReactiveRepositoryJson', async function(req: Request, res: Response) {
  try {
    const filename = path.basename(String(req.body?.filename || "repository.json"));
    const directory = path.join(__dirname, baseReactiveRepositoryPath);
    const filepath = path.join(directory, filename);
    await fs.promises.mkdir(directory, { recursive: true });
    const expectedRevision = req.header("If-Match");
    if (expectedRevision) {
      try {
        const existing = JSON.parse(await fs.promises.readFile(filepath, "utf8"));
        const actualRevision = String(existing?.state?.revision ?? "");
        if (actualRevision !== expectedRevision) {
          res.status(409).send({ Success: false, Error: "revision-conflict", Revision: actualRevision });
          return;
        }
      } catch (error: any) {
    ...
    await atomicWriteJson(filepath, req.body.repository);
    const revision = String(req.body?.repository?.state?.revision ?? "");
    res.setHeader("ETag", revision);
    res.send({ Success: true, Revision: revision });
  } catch (ex) {
    console.error('/api/saveReactiveRepositoryJson', ex);
    res.status(500).send({ Success: false });
  }
});

app.get('/api/loadReactiveRepositoryJson', async function(req: Request, res: Response) {
  try {
    const filename = path.basename(String(req.query.filename || "repository.json"));
    const filepath = path.join(__dirname, baseReactiveRepositoryPath, filename);
    const repository = JSON.parse(await fs.promises.readFile(filepath, "utf8"));
    const revision = String(repository?.state?.revision ?? "");
    res.setHeader("ETag", revision);
    res.send({ Success: true, Revision: revision, Data: { repository } });
  } catch (ex) {
    console.error('/api/loadReactiveRepositoryJson', ex);
