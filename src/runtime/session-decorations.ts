...
export interface SessionDecoration { type: string; owner: string; id: string; range: SearchRange; fill: string; active: boolean; priority: number }
interface Layer { set: SearchMatchSet; visible: boolean; hidden: Set<string>; active?: string; type: string; fill: string; priority: number }
...
export class SessionDecorations {
  ...
  attachMatches(owner: string, set: SearchMatchSet, style: { type?: string; fill?: string; priority?: number } = {}) {
    const previous = this.owners.get(owner);
    this.owners.set(owner, { set, visible: previous?.visible ?? true, hidden: new Set(), type: style.type ?? "editor/search-match", fill: style.fill ?? "#ffd34d", priority: style.priority ?? 0 });
    this.rebuild();
  }
  ...
  private rebuild() {
    ...
    for (const [owner, layer] of this.owners) for (const match of layer.set.matches) {
      ...
      for (const range of match.ranges) (next[range.nodeKey] ??= []).push({ owner, type: layer.type, id: match.id, range, fill: layer.fill, active: layer.active === match.id, priority: layer.priority });
    }
