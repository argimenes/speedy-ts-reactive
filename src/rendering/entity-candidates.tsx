...
export function EntityCandidatesPanel(props: { session: EntityCandidates; bind: () => void }) {
  ...
  return <section class="entity-candidates" aria-label="Mention candidates">
    ...
    <Show when={state.result && (!state.result.exact || state.result.status !== "complete")}><p role="alert">Bulk binding requires a complete search. Narrow the query or scope.</p></Show>
    ...
    <p>Entity: {state.entity ? `${state.entity.name} (${state.entity.id})` : "Choose a name or alias result above."}</p>
    <button type="button" class="entity-candidates__bind" disabled={!session.canBind()} onClick={props.bind}>Bind {session.selected().length} selected mentions{state.entity ? ` to ${state.entity.name}` : ""}</button>
