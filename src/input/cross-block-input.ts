...
export class CrossBlockInput {
  ...
  private key = (event: KeyboardEvent) => {
    ...
    const target = event.target instanceof Element ? event.target : undefined;
    if (target?.closest('input, textarea, select, .document-style-bar, [data-block-selection-handle], [data-block-selection-inspector]')) return;
    const resolved = this.editor.mounts.resolveEvent(event);
    ...
    if (event.key !== "Tab" && !["Shift", "Control", "Meta", "Alt"].includes(event.key)) {
      this.stop(event);
      this.editor.crossText.notice((event.ctrlKey || event.metaKey) && ["c", "x", "v"].includes(event.key.toLowerCase()) ? "Cross-Block text copy/cut/paste is not enabled yet. Escape returns to local text editing." : undefined);
    }
  };
  private guard = (event: Event) => {
    ...
    if (target?.closest('input, textarea, select, .document-style-bar, [data-block-selection-handle], [data-block-selection-inspector]')) return;
    const resolved = this.editor.mounts.resolveEvent(event);
    if (!resolved || !this.editor.crossText.segments[resolved.nodeKey]) return;
    this.stop(event); this.editor.crossText.notice(["copy", "cut", "paste"].includes(event.type) ? "Cross-Block text copy/cut/paste is not enabled yet. Escape returns to local text editing." : undefined);
  };
  dispose() { this.up(); this.observer.disconnect(); for (const [name, fn] of this.listeners) this.document.removeEventListener(name, fn, true); this.editor.crossText.clear(); }
}
