...
try {
  ...
  await evaluate(`(async()=>{
    ...
    const dispose=render(()=>createComponent(ReactiveTreeView,{editor,projection}),host);editor.installGateway(document);const text=Object.values(projection.state.nodes).find(node=>node.payload.id==='text');editor.mounts.get(text.key).root.style.marginLeft='300px';editor.mounts.get(text.key).focus();
  ...
  const opened = await evaluate(`new Promise(resolve=>setTimeout(()=>{const timer=document.querySelector('.reactive-timer'),text=timerCheck.editor.mounts.get(Object.values(timerCheck.projection.state.nodes).find(node=>node.payload.id==='text').key).root,blocks=timerCheck.editor.encodeDocument().children,timerRect=timer.getBoundingClientRect(),textRect=text.getBoundingClientRect();resolve({types:blocks.map(block=>block.type),display:timer.querySelector('output').textContent,width:timerRect.width,right:timerRect.right,textLeft:textRect.left,focused:document.activeElement===timer,close:!!timer.querySelector('[aria-label="Close timer"]'),fits:timer.scrollWidth<=timer.clientWidth&&timer.scrollHeight<=timer.clientHeight});},80))`);
  assert.deepEqual(opened.types, ["standoff-editor-block", "timer-block"]); assert.equal(opened.display, "05:00"); assert.equal(opened.width, 130); assert.equal(opened.right, opened.textLeft); assert.equal(opened.focused, true); assert.equal(opened.close, true); assert.equal(opened.fits, true);
