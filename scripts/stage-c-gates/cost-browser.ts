...
(window as any).run = async (durationSeconds: number) => {
  try {
    ...
    s.repository.subscribeHistoryChanges(e => {
      ...
      if (event) {
        if (inFlight >= 64) throw Error("Gate worker queue cap");
        inFlight++; report.maxInFlight = Math.max(inFlight, report.maxInFlight);
        worker.postMessage({ event, created: Date.now() });
      }
      report.callback.push(performance.now() - start);
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (i === Math.floor(total * .3)) await fetch("/offline", { method: "POST" });
      if (i === Math.floor(total * .4)) await fetch("/online", { method: "POST" });
      const at = performance.now(); s.edit(i); report.edits.push(performance.now() - at);
  ...
  } catch (error: any) { report.errors.push(String(error?.stack ?? error)); report.completed = true; }
};
