...
import { runSchedule } from "./cost-scheduler";
...
const telemetry = () => ({ source: "main", at: Date.now(), progress: report.progress, errors: report.errors, maxInFlight: report.maxInFlight,
  mainStages: stages, worker: report.workerTelemetry, heap: report.heap?.at(-1), queueFailure: report.queueFailure, phase: report.phase, scheduling: observation.snapshot(), acknowledgements: report.acknowledgements });
...
(window as any).run = async (durationSeconds: number, diagnosticPauseMs = 0) => {
  try {
    report.phase = "matrices";
    ...
    report.acknowledgements = { posted: 0, delivered: 0, lastPosted: null, lastDelivered: null };
    ...
    worker.onmessage = ({ data }) => {
      ...
      if (data.captured) {
        if (!sent.has(data.captured)) throw Error("Unexpected or duplicate worker acknowledgement");
        report.acknowledgements.delivered++;
        report.acknowledgements.lastDelivered = { commitId: data.captured, workerPostedAt: data.capturedAt, sequence: data.captureSequence, mainReceivedAt: epoch(), boundary: observation.boundary() };
    ...
    s.repository.subscribeHistoryChanges(e => {
      ...
      if (event) {
        ...
        report.acknowledgements.posted++;
        report.acknowledgements.lastPosted = { commitId: event.commitId, postedAt, boundary: observation.boundary() };
    ...
    report.phase = "sustained";
    await runSchedule(total, 200, { now: () => performance.now(),
      wait: (delay, index, target) => observation.wait(delay, "edit", { index, target }),
      edit: async (i, target) => {
      if (report.errors.length) throw Error(report.errors.join("\n"));
      if (diagnosticPauseMs && i === 10) observation.span("diagnosticBusyTask", () => {
        const until = performance.now() + diagnosticPauseMs;
        while (performance.now() < until) { /* Explicit calibration, never qualification. */ }
      });
      ...
      measure("main.scheduleLateness", Math.max(0, performance.now() - target));
      const at = performance.now(); observation.span("edit", () => s.edit(i)); report.edits.push(performance.now() - at); measure("main.edit", performance.now() - at);
    ...
    for (let i = 0; i < 1200 && !finish; i++) { worker.postMessage({ kind: "finish" }); await observation.wait(100, "drain"); }
    if (!finish) throw Error("Gate drain deadline");
    report.drainMs = performance.now() - drain;
    const repository = s.repository as any;
    report.undoEntryCount = repository.undoStack.length;
    report.undoStorage = repository.historyStorage?.diagnostics([...repository.undoStack, ...repository.redoStack]);
    report.phase = "independentOracle"; await publishTelemetry();
    report.oracleWire = observation.span("independentOracle", () => encodeWire(s.final()));
    ...
    report.worker = finish; worker.terminate(); report.phase = "completed";
    await observation.wait(0, "finalObservation"); report.scheduling = observation.snapshot();
    report.completed = true; observation.stop();
  } catch (error: any) { report.errors.push(String(error?.stack ?? error)); report.scheduling = observation.snapshot(); report.completed = true; observation.stop(); await publishTelemetry(); }
};
