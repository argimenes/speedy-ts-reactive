...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.url === '/telemetry') { telemetry.push(JSON.parse(body)); if (telemetry.length > 2000) telemetry.shift(); response.end('{}'); return; }
...
try {
  ...
  const output = { gate: 'G3 command/worker/outbox/server feasibility', hostResources, date: new Date().toISOString(), node: process.version, browser: page.version.product,
    durationSeconds: duration, revisions: accepted, instrumentation: { main: result.mainStages, undoStorage: result.undoStorage, worker: result.worker.pipeline, server: { stages: serverStages, rejectedAppends, networkTransitions }, telemetry }, baselineBytes: result.baselineBytes, matrices,
  ...
  await writeFile(path, JSON.stringify({ gate: 'G3 command/worker/outbox/server feasibility', hostResources, passed: false, date: new Date().toISOString(), node: process.version,
    browser: browserVersion, requestedSeconds: duration, acceptedRevisions: accepted, lastStatus, partialReport, instrumentation: { server: { stages: serverStages, rejectedAppends, networkTransitions }, telemetry }, error: String(error.stack ?? error), diagnostics: browser.diagnostics }, null, 2) + '\n');
  throw error;
